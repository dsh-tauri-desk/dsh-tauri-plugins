...
describe('state round-trip', () => {
  it('returns empty for missing and corrupt files', () => {
    ...
    mkdirSync(join(corrupt, 'dsh-tauri-panel-extension'), { recursive: true })
    writeFileSync(join(corrupt, 'dsh-tauri-panel-extension', 'state.json'), '{oops')
...
describe('removeSkillRoot with link material', () => {
  it('deletes the junction wrapper without touching the linked source', () => {
    ...
    expect(readFileSync(join(home, 'dsh-tauri-panel-extension', 'state.json'), 'utf8')).not.toContain('linked-source')
  })
})
