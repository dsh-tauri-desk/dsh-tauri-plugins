...
export function withConnectionAuth(connection: ConnectionGate | undefined, handler: RouteHandler): RouteHandler {
  if (typeof connection?.requestRejection !== 'function')
    throw new TypeError('dsh-tauri-worktree requires the DSH connection authentication gate')
...
export function routeHandler(fn: RouteFunction, { mutate = false }: { mutate?: boolean } = {}): (req: IncomingMessage, res: ServerResponse) => Promise<void> {
