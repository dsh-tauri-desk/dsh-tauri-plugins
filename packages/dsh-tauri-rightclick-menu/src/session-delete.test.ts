...
const DSH_HOME = join(tmpdir(), 'dsh-session-delete-test-home')
...
function sessionFile(sessionId: string): string {
  return join(SESSIONS, sessionId, `${sessionId}.jsonl`)
}
...
describe('safeSessionDirectory', () => {
  ...
  it('accepts the exact session directory under DSH_HOME/sessions', () => {
    ...
    const sessionDir = join(SESSIONS, 'abc123')
  ...
  it('rejects a directory outside DSH_HOME/sessions', () => {
    process.env.DSH_HOME = DSH_HOME
    expect(() => safeSessionDirectory(location(join(DSH_HOME, 'other', 'file.jsonl')), 'abc123'))
      .toThrow(/unsafe session directory/)
  })

  it('rejects a parent directory of the sessions root', () => {
    process.env.DSH_HOME = DSH_HOME
    expect(() => safeSessionDirectory(location(join(DSH_HOME, 'file.jsonl')), 'abc123'))
      .toThrow(/unsafe session directory/)
  })
