...
项目选择 / 取消归档），并在官方工作区浏览器的「删除工作区」菜单旁
追加「归档工作区」入口。
...
- **永久删除安全门**：当前 DSH 尚未提供由宿主协调 Agent、持久化写入器、会话
  注册表与磁盘数据的删除事务，因此永久删除暂时不可用。归档与取消归档继续正常
  工作，已归档数据会保留。旧客户端调用删除端点时会收到明确的 `503`，且不会
  改写宿主状态或磁盘。
- **加载态**：变更（取消归档）进行中时动作按钮禁用并弹 loading toast，
  完成后消失；失败在页面顶部显示错误。
...
- 宿主公开 API 只有 `archiveSession`（没有 unarchive），因此「取消归档」经由
  注册表内部状态机（`enqueueOperation` + `requireState` + `setState`）
...
| POST | `/delete` | 旧客户端兼容端点；返回 `503`，不执行任何变更 |
| POST | `/delete-workspace` | 旧客户端兼容端点；返回 `503`，不执行任何变更 |
| POST | `/clear` | 旧客户端兼容端点；返回 `503`，不执行任何变更 |
