 ...
 * 模块做增广；不可解析的 renderer 见 ./renderer.d.ts 的脚本式 ambient）。
 *
 ...
 *
 * 注：'sidebar' / 'conversation' / 'sidebar.panel.action' 的 SlotMap 声明分别
 * 归属 ui-layout / ui-sidebar（不在本插件类型图、未提升到根 node_modules），
 * 无法在保留模块同一性的前提下在此增广——注册处改用显式 cast（先例：
 * dsh-tauri-ui 的 trigger.tsx）。组件 props 仍由本地接口提供类型。
 *
